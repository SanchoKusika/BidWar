...
export function PaidMobile({ nav }: PaidMobileProps) {
  ...
  const pay = async () => {
    ...
    try {
      ...
      // Неизвестный исход — это предупреждение, а не отказ: сказать «не
      // прошло» тем же откликом, что и при честном отказе, значит соврать
      // телом там, где текст как раз признаётся, что мы не знаем.
      haptic(error instanceof PaymentOutcomeUnknownError ? 'warning' : 'error');
      setPayError(
    ...
    if (outcome.status === 'pending') {
      ...
      // Не успех и не отказ: платёж создан, ответа провайдера ещё нет.
      // Отдельный отклик именно потому, что человек в этот момент не понимает,
      // что стало с деньгами, — молчание здесь худший из вариантов.
      haptic('warning');
    ...
    //
    // У атаки он другой: это единственное действие в продукте, которое бьёт по
    // другому человеку, и «получилось» тут слабее правды. Удар ощущается ударом.
    haptic(pending.kind === 'attack' ? 'heavy' : 'success');
