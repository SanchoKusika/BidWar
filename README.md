...
Не работает — **денег в продукте нет ни одной строкой**: платёжный слой не начат,
поэтому платного входа, Raise и Attack не существует. Ближайший срез (1.5)
заводит платёжный мок и Raise.
...
- шторки `RaiseSheet`, `AttackSheet`, `VoteSheet`, `PaySheet`, `ResultSheet`
  свёрстаны, но никуда не подключены;
- экран проекта свёрстан и пока недостижим: витрина не передаёт на него переход.

> **Автотестов в репозитории нет ни одного.** CI проверяет, что код собирается, а
> не что он правильно считает. Раннер и первые тесты ставятся вместе с платёжным
> моком — на нём же они и будут гоняться, реальные платежи в CI никто не пустит.
...
(`resolve_telegram_identity`), дедупликация кликов (`register_project_click`) и
агрегаты категорий (`category_stats`). Часть логики опущена в базу намеренно —
...
src/
  ...
  features/       пусто: наполнится в 1.5–1.7 вместе с raise/attack/vote
