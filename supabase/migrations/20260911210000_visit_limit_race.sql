-- Предел переходов считается ПОСЛЕ блокировки, а не до.
--
-- Находка ревью PR #54. Счёт стоял до `for update` по `users`, а значит не
-- сериализовался: десять одновременных кликов по разным проектам — десять
-- транзакций на READ COMMITTED, ни одна не видит незакоммиченных строк
-- остальных, все читают ноль, все проходят проверку и все платят. Предел,
-- который не держит под нагрузкой, — это подпись, а не предел.
--
-- Блокировка строки `users` в этой же хранимке уже выстраивает вызовы одного
-- человека в очередь (она нужна для баланса), поэтому достаточно перенести счёт
-- за неё — своей блокировки предел не требует.
...
begin
  ...
  -- Суточный предел переходов — строго за блокировкой: до неё параллельные
  -- вызовы читали ноль каждый и проходили все разом.
